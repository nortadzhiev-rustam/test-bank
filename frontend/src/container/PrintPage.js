...
const Android12Switch = styled(Switch)(({ theme }) => ({
  ...
  "& .MuiSwitch-track": {
    ...
    "&:before": {
      backgroundImage: `url('data:image/svg+xml;utf8,<svg xmlns="http://w3.org/2000/svg" height="16" width="16" viewBox="0 0 24 24"><path fill="${encodeURIComponent(
...
const PrintPage = ({ showNav, setShowNav }) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5001/api/v1/test/${id}`)
      .then((res) => {
        if (res.status === 200) {
          setData(res.data);
